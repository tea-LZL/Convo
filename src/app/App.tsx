...
function AppShell() {
  ...
  useEffect(() => {
    registerMany([
      ...
      { id: "settings", combo: "ctrl+,", action: () => navigate("/settings/general"), description: "Settings" },
      { id: "focus-input", combo: "ctrl+/", action: () => window.dispatchEvent(new CustomEvent("convo:focus-input")), description: "Focus input" },
    ]);
  }, [registerMany, togglePalette, isNarrow, navigate]);
  ...
  return (
    <div className="h-full flex bg-bg text-text relative overflow-hidden">
      ...
      <main
        ...
        className="flex-1 min-w-0 h-full flex flex-col relative animate-fade-in"
      >
        <RouteErrorBoundary>
          <Routes>
