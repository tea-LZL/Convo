...
describe("api", () => {
  describe("parameterized command wrappers", () => {
    ...
    const cases: Case[] = [
      ...
      {
        name: "queueMemoryReview",
        method: "queueMemoryReview",
        args: ["session-1"],
        expectedCommand: "queue_memory_review",
        expectedPayload: { sessionId: "session-1" },
      },
      {
        name: "finishMemoryReview includes attempt",
        method: "finishMemoryReview",
        args: ["review-1", 2, []],
        expectedCommand: "finish_memory_review",
        expectedPayload: { id: "review-1", attempt: 2, facts: [] },
      },
      {
        name: "failMemoryReview includes attempt",
        method: "failMemoryReview",
        args: ["review-1", 2, "offline"],
        expectedCommand: "fail_memory_review",
        expectedPayload: { id: "review-1", attempt: 2, error: "offline" },
      },
      {
        name: "retryMemoryReview",
        method: "retryMemoryReview",
        args: ["review-1"],
        expectedCommand: "retry_memory_review",
        expectedPayload: { id: "review-1" },
      },
      {
        name: "runCompare",
        method: "runCompare",
